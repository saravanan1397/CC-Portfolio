  ...
  state.cards = state.cards.map((card) => {
    ...
    legacyBenefits.forEach((benefit) => {
      ...
      if (isPointBenefit(benefit)) {
        ...
        entry.points += toNumber(benefit.pointsAmount);
...
function interpretAiIntent(query) {
  ...
  const resolvedCard = cardMatch.card || trainerHint.card || null;
...
function getRpSpendRedemptionAmount(rpSpend) {
  ...
  if (getCardById(sourceCardId)?.benefits?.some((benefit) => benefit?.type === welcomeBenefitPointsType)) {
    return totalPoints;
  }
...
function formatCardBenefitsHtml(card) {
  ...
  const welcomeLines = hasWelcomePoints ? `
    ...
    <span class="benefit-line benefit-line-derived">
      ...
      <span class="benefit-line-value">${formatMixedValueHtml(allocation.welcomeRedeemedValue, allocation.welcomeRedeemedPoints)}</span>
    </span>
  ` : "";
