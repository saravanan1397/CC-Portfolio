...
document.addEventListener("click", (e) => {
  ...
  els.rpSpendUnredeemedOnly?.addEventListener("change", () => {
    state.rpSpendUnredeemedOnly = els.rpSpendUnredeemedOnly.checked;
    rpSpendsAllExpanded = false;
    renderRpSpends();
  });
...
function normalizeIntlTravelExpense(expense) {
  return {
    ...
    expenseType: String(expense?.expenseType || "Personal").trim() || "Personal",
...
function getIntlTravelTripCurrencies(trip) {
  ...
  (trip?.expenses || []).forEach((expense) => {
    ...
    totals.set(currency, (totals.get(currency) || 0) + toNumber(expense.localAmount));
  });
  return Array.from(totals.entries())
    .sort((a, b) => b[1] - a[1])
    .map(([currency, amount]) => `${currency} ${formatIntlTravelAmount(amount)}`)
    .join(" | ");
}
...
function renderIntlTravelDetail(trip) {
  ...
  const total = getIntlTravelTripTotal(trip);
  const markupTotal = trip.expenses.reduce((sum, expense) => sum + toNumber(expense.markupFee), 0);
  const currencyLabel = getIntlTravelTripCurrencies(trip);
  els.intlTravelDetailTitle.textContent = trip.destination;
  els.intlTravelDetailSummary.textContent = `${trip.expenses.length} ${trip.expenses.length === 1 ? "expense" : "expenses"} | ${formatMoney(total)} total${markupTotal ? ` | ${formatMoney(markupTotal)} fees` : ""}${currencyLabel ? ` | ${currencyLabel}` : ""}`;

  if (!trip.expenses.length) {
    els.intlTravelExpenseTable.innerHTML = `
      <div class="intl-travel-empty-state">
        <h3>No expenses recorded</h3>
        <p class="empty-copy">Add the first expense for this destination.</p>
      </div>
    `;
    return;
  }
...
function openIntlTravelExpenseForm(tripId = "") {
  intlTravelDetailTripId = "";
...
async function saveIntlTravelExpenseFromForm(event) {
  ...
  trip.expenses.push(normalizeIntlTravelExpense({
    id: createId(),
  ...
  await saveState();
  render();
  showToast("Trip expense saved.");
}

async function handleIntlTravelExpenseAction(event) {
  const button = event.target.closest?.('[data-intl-travel-action="delete-expense"]');
...
function resetRpSpendFilters() {
  ...
  if (els.rpSpendUnredeemedOnly) els.rpSpendUnredeemedOnly.checked = false;
  rpSpendsAllExpanded = false;
  renderRpSpends();
}
...
function showCashPopup() {
  ...
  if (!cashCards.length) {
    ...
    content.innerHTML = cashCards.map((card) => {
      ...
      return `
        <article class="cash-benefit-popup-row">
