...
function savePprManualPoints() {
  ...
  const existingEntry = isEditing
    ...
    : null;
  const value = toNumber(existingEntry?.value || 0);
  const notes = String(existingEntry?.notes || "").trim();
  const date = String(existingEntry?.date || "").trim();
          ...
          <article class="ppr-detail-card">
            <div class="ppr-detail-card-head">
              ...
              <div class="ppr-detail-actions">
                ...
                ${group.isManual ? `<button type="button" class="ghost-button" data-ppr-edit-manual="${escapeAttribute(group.manualEntry.id)}">Edit</button>` : ""}
              </div>
            </div>
