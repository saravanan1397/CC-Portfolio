...
function renderIntlTravelDetail(trip) {
  ...
  const total = expenses.reduce((sum, expense) => sum + toNumber(expense.inrAmount) + toNumber(expense.markupFee), 0);
  const markupTotal = expenses.reduce((sum, expense) => sum + toNumber(expense.markupFee), 0);
  els.intlTravelDetailTitle.textContent = trip.destination;
  els.intlTravelDetailSummary.innerHTML = `
    <span>${expenses.length} ${expenses.length === 1 ? "expense" : "expenses"}${selectedType === "all" ? "" : ` · ${escapeHtml(selectedType)}`}</span>
    <strong>${escapeHtml(formatMoney(total))} total</strong>
    ${markupTotal ? `<span>${escapeHtml(formatMoney(markupTotal))} fees</span>` : ""}
  `;
  renderIntlTravelCurrencySummary(trip, expenses);
  ...
  if (!expenses.length) {
    els.intlTravelExpenseTable.innerHTML = `
      <div class="intl-travel-empty-state">
        <h3>${selectedType === "all" ? "No expenses recorded" : "No matching expenses"}</h3>
        <p class="empty-copy">${selectedType === "all" ? "Add the first expense for this destination." : "Choose another type to see more expenses."}</p>
      </div>
    `;
    return;
  }
  ...
  els.intlTravelExpenseTable.innerHTML = `
    ...
    ${sortedExpenses.map((expense) => `
      <article class="intl-travel-expense-row">
        ...
        <span class="intl-travel-cell">${escapeHtml([expense.paymentMethod, expense.paymentSource].filter(Boolean).join(" | "))}</span>
...
function openIntlTravelConversionForm() {
  const trip = state.intlTravelTrips.find((item) => item.id === intlTravelDetailTripId);
  if (!trip || !els.intlTravelConversionForm) return;

  els.intlTravelConversionForm.reset();
  if (els.intlTravelConversionCurrency) els.intlTravelConversionCurrency.value = "MYR";
...
async function saveIntlTravelConversionFromForm(event) {
  ...
  trip.conversions.push(normalizeIntlTravelConversion({
    id: createId(),
  ...
  await saveState();
  render();
  showToast("Currency conversion saved.");
}
...
function resetIntlTravelExpenseForm() {
  ...
  els.intlTravelDestination.readOnly = false;
  if (els.intlTravelCurrency) els.intlTravelCurrency.value = "MYR";
  if (els.intlTravelPaymentMethod) els.intlTravelPaymentMethod.value = "Cash";
  if (els.intlTravelPaymentSourceCardSelect) els.intlTravelPaymentSourceCardSelect.value = "";
  if (els.intlTravelExpenseType) els.intlTravelExpenseType.value = "Personal";
  if (els.intlTravelPhase) els.intlTravelPhase.value = "On-trip";
  if (returnTripId) {
