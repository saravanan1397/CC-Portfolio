...
function cacheElements() {
  Object.assign(els, {
    ...
    intlTravelExpenseFilter: document.getElementById("intlTravelExpenseFilter"),
...
document.addEventListener("click", (e) => {
  ...
  els.intlTravelExpenseFilter?.addEventListener("change", handleIntlTravelDetailFilterChange);
...
async function loadState() {
  try {
    ...
    if (snap.exists()) {
      ...
      if (migratedLegacyRp || removedDuplicateLegacyRp) {
        await saveState();
      }
...
function migrateLegacyPointsRedeemedBenefitsToRpSpends() {
  ...
  state.cards = state.cards.map((card) => {
    ...
    legacyBenefits.forEach((benefit) => {
      ...
      if (isPointBenefit(benefit)) entry.points += amount;
      else entry.pointsValue += amount;
...
function syncRpRedeemedBenefitsFromSpends() {
  ...
  state.rpSpends.forEach((rpSpend) => {
    ...
    if (isUnredeemedPointsRecord(rpSpend)) {
      ...
      if (hasUnredeemedSource) cardTotals.unredeemedPoints -= totalPoints;
    } else {
...
function renderDashboard() {
  ...
  if (els.dashboardIntlTravelValue) {
    els.dashboardIntlTravelValue.textContent = formatMoney(intlTravelTotals.totalInr);
  }

  if (els.dashboardIntlTravelHint) {
    els.dashboardIntlTravelHint.textContent = intlTravelTotals.tripCount
      ? `${intlTravelTotals.tripCount} ${intlTravelTotals.tripCount === 1 ? "trip" : "trips"} - ${intlTravelTotals.expenseCount} expenses`
      : "No trips recorded";
  }
}
...
function getIntlTravelExpenseSign(expense) {
  return isIntlTravelRefund(expense) ? -1 : 1;
}

function getIntlTravelExpenseSignedLocalAmount(expense) {
  return getIntlTravelExpenseSign(expense) * toNumber(expense?.localAmount);
}

function getIntlTravelExpenseSignedInrAmount(expense) {
  return getIntlTravelExpenseSign(expense) * (toNumber(expense?.inrAmount) + toNumber(expense?.markupFee));
}
...
function getIntlTravelTripTotal(trip) {
  return (trip?.expenses || []).reduce((sum, expense) => sum + getIntlTravelExpenseSignedInrAmount(expense), 0);
}
...
function getIntlTravelTotals() {
  ...
  return {
    ...
    expenseInr: expenses.reduce((sum, expense) => sum + getIntlTravelExpenseSign(expense) * toNumber(expense.inrAmount), 0),
    markupInr: expenses.reduce((sum, expense) => sum + getIntlTravelExpenseSign(expense) * toNumber(expense.markupFee), 0),
    totalInr: expenses.reduce((sum, expense) => sum + getIntlTravelExpenseSignedInrAmount(expense), 0),
  };
}
...
function getIntlTravelCurrencyTotals(expenses = []) {
  ...
  expenses.forEach((expense) => {
    ...
    current.localAmount += getIntlTravelExpenseSignedLocalAmount(expense);
    current.inrAmount += getIntlTravelExpenseSignedInrAmount(expense);
...
function getIntlTravelPhaseTotals(expenses = []) {
  ...
  return phases.map((phase) => {
    ...
    return {
      ...
      totalInr: phaseExpenses.reduce((sum, expense) => sum + getIntlTravelExpenseSignedInrAmount(expense), 0),
      currencies: getIntlTravelCurrencyTotals(phaseExpenses),
    };
  });
}
...
function renderIntlTravelDetailFilterOptions(trip) {
  ...
  setIntlTravelFilterOptions(
    els.intlTravelExpenseFilter,
    expenses.map((expense) => expense.description || "Untitled expense"),
    "All expenses"
  );
  setIntlTravelFilterOptions(
...
function getIntlTravelExpenseDisplayOrder(trip) {
  const hasCustomOrder = (trip?.expenses || []).some((expense) => Number.isFinite(Number(expense.displayOrder)));
  return [...(trip?.expenses || [])].sort((a, b) => {
    ...
    return compareIntlTravelExpensesByDate(a, b);
  });
}
...
async function handleIntlTravelExpenseDrop(event) {
  ...
  const orderedExpenses = getIntlTravelExpenseDisplayOrder(trip);
  const sourceIndex = orderedExpenses.findIndex((expense) => expense.id === sourceId);
  const targetIndex = orderedExpenses.findIndex((expense) => expense.id === row.dataset.expenseId);
  if (sourceIndex < 0 || targetIndex < 0) return;

  const [movedExpense] = orderedExpenses.splice(sourceIndex, 1);
  const insertionIndex = sourceIndex < targetIndex ? targetIndex - 1 : targetIndex;
  orderedExpenses.splice(insertionIndex, 0, movedExpense);
  orderedExpenses.forEach((expense, index) => {
    expense.displayOrder = index;
  });
  trip.expenses = orderedExpenses;
  intlTravelDraggingExpenseId = "";

  await saveState();
  render();
  showToast("Expense order updated.");
}
  ...
  els.intlTravelTripCards.innerHTML = trips.map((trip) => {
    ...
    return `
      <article class="intl-travel-trip-card" data-intl-trip-id="${escapeAttribute(trip.id)}" tabindex="0" role="button" aria-label="Open ${escapeAttribute(trip.destination)} expenses">
        ...
        <div class="intl-travel-trip-total ${getIntlTravelTripTotal(trip) < 0 ? "intl-travel-negative-value" : ""}">${escapeHtml(formatMoney(getIntlTravelTripTotal(trip)))}</div>
...
function showIntlTravelHome() {
  ...
  if (els.intlTravelExpenseFilter) els.intlTravelExpenseFilter.value = "all";
      ...
      <div class="intl-travel-currency-card-grid">
        ${currencyTotals.map(({ currency, localAmount, inrAmount }) => `
          <article class="intl-travel-currency-card">
            ...
            <small class="${inrAmount < 0 ? "intl-travel-negative-value" : ""}">INR equivalent ${escapeHtml(formatMoney(inrAmount))}</small>
          </article>
        `).join("")}
      </div>
    ` : `
      ...
      <div class="intl-travel-phase-card-grid">
        ${phaseTotals.map((phase) => `
          <article class="intl-travel-phase-card">
            ...
            <strong class="${phase.totalInr < 0 ? "intl-travel-negative-value" : ""}">${escapeHtml(formatMoney(phase.totalInr))}</strong>
...
function renderIntlTravelDetail(trip) {
  ...
  const selectedExpense = els.intlTravelExpenseFilter?.value || "all";
  ...
  const filteredExpenses = expenses.filter((expense) => {
    const expenseLabel = expense.description || "Untitled expense";
    const paymentLabel = getIntlTravelPaymentLabel(expense);
    return (selectedExpense === "all" || expenseLabel === selectedExpense)
      && (selectedCategory === "all" || expense.category === selectedCategory)
      && (selectedPayment === "all" || paymentLabel === selectedPayment);
  });
  const total = filteredExpenses.reduce((sum, expense) => sum + getIntlTravelExpenseSignedInrAmount(expense), 0);
  const markupTotal = filteredExpenses.reduce((sum, expense) => sum + getIntlTravelExpenseSign(expense) * toNumber(expense.markupFee), 0);
  const activeFilters = [
    selectedType !== "all" ? selectedType : "",
    selectedExpense !== "all" ? selectedExpense : "",
  ...
  els.intlTravelDetailSummary.innerHTML = `
    ...
    <strong class="${total < 0 ? "intl-travel-negative-value" : ""}">${escapeHtml(formatMoney(total))} total</strong>
  ...
  els.intlTravelExpenseTable.innerHTML = `
    ...
    ${expensesToRender.map((expense) => `
      <article class="intl-travel-expense-row" draggable="true" data-expense-id="${escapeAttribute(expense.id)}">
        ...
        <strong class="intl-travel-cell intl-travel-local-value ${getIntlTravelExpenseSignedLocalAmount(expense) < 0 ? "intl-travel-negative-value" : ""}">${escapeHtml(formatIntlTravelCurrencyAmount(expense.currency, getIntlTravelExpenseSignedLocalAmount(expense)))}</strong>
        <strong class="intl-travel-cell intl-travel-inr-value ${getIntlTravelExpenseSignedInrAmount(expense) < 0 ? "intl-travel-negative-value" : ""}">${escapeHtml(formatMoney(getIntlTravelExpenseSignedInrAmount(expense)))}</strong>
...
async function saveIntlTravelExpenseFromForm(event) {
  ...
  if (editingExpenseId) {
    ...
    expenseData.createdAt = trip.expenses[expenseIndex].createdAt;
    expenseData.displayOrder = trip.expenses[expenseIndex].displayOrder;
  ...
  return new Promise((resolve) => {
    ...
    removeBtn?.addEventListener("click", async () => {
      ...
      const sourceCard = getCardById(String(originatingCardSelect?.value || "").trim())
        || { issuer: "Partner program", name: partnerNameInput?.value.trim() || "redemption" };
...
function getRpSpendRedemptionAmount(rpSpend) {
  ...
  const totalPoints = getRpSpendTotalPoints(rpSpend);
  if (isPartnerProgramRpSpend(rpSpend)) {
    // When points were entered through the Redeem popup, use that tracked
    // redeemed amount for balance validation. Otherwise support manually
    // entered Partner Program points from the main Points textbox.
    const redeemedPoints = toNumber(rpSpend.redeemedPoints);
    return redeemedPoints > 0 ? redeemedPoints : totalPoints;
  }
...
function restoreRpSpendRedemption(rpSpend) {
  const pointsToRestore = getRpSpendRedemptionAmount(rpSpend);
  const sourceCardId = getRpSpendRedeemedSourceCardId(rpSpend);

  if (pointsToRestore > 0 && sourceCardId && !getUnredeemedPointsSourceRecord(sourceCardId)) {
    restoreCardPointRedemption(sourceCardId, pointsToRestore);
  }
...
function applyCardPointRedemption(cardId, redeemPoints) {
  ...
  const sourceRecord = getUnredeemedPointsSourceRecord(cardId);
  // An explicit Unredeemed Points source is consumed by the RP spend row
  // itself. This function runs after that row has been inserted, so checking
  // the balance again here would see 950 - 476 = 474 and reject the original
  // 476-point redemption. The pre-save validation already checked the amount
  // against the balance before the new row was added.
  if (sourceRecord) {
    return { ok: true, currentUnredeemed: getUnredeemedSourceBalance(sourceRecord) };
  }
  ...
  if (pointsToRedeem > currentUnredeemed) {
    return { ok: false, error: `You can redeem up to ${formatPoints(currentUnredeemed)}.` };
  }
  ...
  const updatedBenefits = (card.benefits || []).map((benefit) => {
    ...
    remaining -= deducted;
    return { ...benefit, amount: currentAmount - deducted };
  });

  if (remaining > 0) return { ok: false, error: "Unable to apply the full redemption amount." };
  state.cards[cardIndex] = { ...card, benefits: updatedBenefits };
...
function renderRpSpends() {
  ...
  const filteredGroups = sortedGroups.filter((group) => {
    if (showUnredeemedOnly && !group.items.some((item) => getRpSpendRemainingPoints(item) > 0)) {
      return false;
    }
...
els.pointsValue.title = "Click to view point breakdown";
  ...
  els.benefitValue.style.color = totals.cashBenefitsNet > 0 ? "#10b981" : totals.cashBenefitsNet < 0 ? "#ef4444" : "#f8fafc";
...
function getCardTotals(card) {
  ...
  const cashBenefitNet = benefits - fees;
...
function getRpPointsUsageTotals() {
  return state.rpSpends.reduce(
    (totals, rpSpend) => {
      // "Not Spent" is driven only by the explicit Unredeemed Points
      // checkbox. Its remaining balance already excludes points redeemed
      // from that source by later RP spend rows.
      if (isUnredeemedPointsRecord(rpSpend)) {
...
function getPartnerProgramPurchaseEarnedPoints(items = []) {
  ...
  // pointsReceived is a product-level override and may be copied to every
  // payment row. Use it once when explicitly entered; otherwise calculate
  // earned points from each partner row's points and transfer ratio.
  const explicitReceived = partnerItems.find((item) => toNumber(item.pointsReceived) > 0);
  if (explicitReceived) return toNumber(explicitReceived.pointsReceived);
