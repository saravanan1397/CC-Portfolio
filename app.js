...
function normalizeRpSpend(spend) {
  ...
  const normalizedPoints = toNumber(spend.points);
  ...
  const autoPointsReceived = partnerTransferRatio && parsedRatio && normalizedPoints > 0
    ? computePartnerTransferPoints(normalizedPoints, parsedRatio)
  ...
  return {
    ...
    partnerProgram: spend.partnerProgram === true || spend.cardId === partnerProgramPlatformValue,
...
function syncRpRedeemedBenefitsFromSpends() {
  ...
  state.rpSpends.forEach((rpSpend) => {
    ...
    const hasPartnerSource = isPartnerProgramRpSpend(rpSpend) && !!sourceCardId;
    if (toNumber(rpSpend.pointsValue) > 0 || hasPartnerSource) {
  ...
  state.cards = state.cards.map((card) => {
    ...
    if (cardTotals && (cardTotals.pointsKnownValue > 0 || cardTotals.knownValuePoints > 0)) {
      benefits.push({
...
function updatePartnerProgramOriginInfo(selectEl, infoEl) {
  ...
  infoEl.innerHTML = `
    ...
    <strong style="font-size:1.4rem; color:#f8fafc;">${escapeHtml(formatPoints(totals.points))}</strong>
    <p style="margin:0; color:#94a3b8; font-size:0.88rem; line-height:1.45;">
      You are about to redeem points from this card.
      If these points have been transferred to a partner program, consider updating the card's Unredeemed Points after saving.
    </p>
  `;
}
...
function showPartnerProgramTransferPrompt(initial = {}) {
  return new Promise((resolve) => {
    ...
    const refreshInfo = () => updatePartnerProgramOriginInfo(originatingCardSelect, infoEl);
          ...
          ${group.items.map(item => `
            <div class="benefit-line">
              ...
              <span class="benefit-line-meta">
                ${toNumber(item.points) > 0 ? `${formatPoints(item.points)}` : ''}
                ${toNumber(item.cardPaid) > 0 ? ` Card: ${formatMoney(item.cardPaid)}` : ''}
                ${toNumber(item.voucherPaid) > 0 ? `Voucher: ${formatMoney(item.voucherPaid)}` : ''}
              </span>
              ${isPartnerProgramRpSpend(item) && (item.partnerName || item.purchasedFrom) ? `<span class="card-meta" style="display:block; margin-top:4px;">Partner: ${escapeHtml(item.partnerName || item.purchasedFrom)}</span>` : ""}
...
function getPartnerProgramPoints(rpSpend) {
  ...
  if (ratio && toNumber(rpSpend?.points) > 0) {
    return computePartnerTransferPoints(rpSpend.points, ratio);
  }
  return toNumber(rpSpend?.points || 0);
}
...
function renderPprWidget() {
  ...
  if (els.dashboardPprHint) {
    els.dashboardPprHint.textContent = summary.purchaseCount
      ? `${summary.purchaseCount} partner entr${summary.purchaseCount === 1 ? "y" : "ies"} | ${summary.partnerCount} partner${summary.partnerCount === 1 ? "" : "s"}`
      : "Hotel and airline partner points";
  }
  ...
  // Create table header
  let html = `
    <div class="cards-table ppr-table" style="width: 100%; gap: 0; margin: 0;">
      <div class="table-head" style="padding: 12px 16px; gap: 0; margin: 0; margin-bottom: 0;">
  ...
  summary.partnerRows.forEach((row) => {
    ...
    html += `
      <article class="card-row ppr-partner-row" style="padding: 12px 16px; gap: 0; align-items: center; margin: 0;">
