...
function cacheElements() {
  Object.assign(els, {
    ...
    widgetActivitySubtitle: document.getElementById("widgetActivitySubtitle"),
    widgetActivityList: document.getElementById("widgetActivityList"),
    closeWidgetActivityBtn: document.getElementById("closeWidgetActivityBtn"),
    closeWidgetActivityFooterBtn: document.getElementById("closeWidgetActivityFooterBtn"),
    ...
    dashboardIncomeNet: document.getElementById("dashboardIncomeNet"),
    dashboardIncomeInterest: document.getElementById("dashboardIncomeInterest"),
    dashboardIncomeMisc: document.getElementById("dashboardIncomeMisc"),
    dashboardIncomeSpends: document.getElementById("dashboardIncomeSpends"),
    dashboardIncomeHint: document.getElementById("dashboardIncomeHint"),
    ...
    backFromInterestIncomeBtn: document.getElementById("backFromInterestIncomeBtn"),
    incomeEntryForm: document.getElementById("incomeEntryForm"),
    incomeEditingId: document.getElementById("incomeEditingId"),
    incomeEditingBadge: document.getElementById("incomeEditingBadge"),
    incomeEntryType: document.getElementById("incomeEntryType"),
    incomeBankSelect: document.getElementById("incomeBankSelect"),
    incomeNewBankField: document.getElementById("incomeNewBankField"),
    incomeNewBankName: document.getElementById("incomeNewBankName"),
    incomeFrequency: document.getElementById("incomeFrequency"),
    incomeYear: document.getElementById("incomeYear"),
    incomeQuarter: document.getElementById("incomeQuarter"),
    incomeMiscSource: document.getElementById("incomeMiscSource"),
    incomeSpendScope: document.getElementById("incomeSpendScope"),
    incomeSpendBankField: document.getElementById("incomeSpendBankField"),
    incomeSpendBank: document.getElementById("incomeSpendBank"),
    incomeAmount: document.getElementById("incomeAmount"),
    incomeDescription: document.getElementById("incomeDescription"),
    incomeNotes: document.getElementById("incomeNotes"),
    clearIncomeEntryBtn: document.getElementById("clearIncomeEntryBtn"),
    saveIncomeEntryBtn: document.getElementById("saveIncomeEntryBtn"),
    incomeRenderInterest: document.getElementById("incomeRenderInterest"),
    incomeRenderMisc: document.getElementById("incomeRenderMisc"),
    incomeRenderSpends: document.getElementById("incomeRenderSpends"),
    incomeRenderNet: document.getElementById("incomeRenderNet"),
    incomeBankTotals: document.getElementById("incomeBankTotals"),
    incomeLedgerBlocks: document.getElementById("incomeLedgerBlocks"),
...
document.addEventListener("keydown", (e) => {
  ...
  els.widgetActivityModal?.addEventListener("click", (event) => {
    if (event.target === els.widgetActivityModal) closeWidgetActivityModal();
  });
  document.addEventListener("keydown", handleWidgetActivityKeydown);
...
const INTEREST_LEDGER_SEED_VERSION = "spreadsheet-through-q3-2026-v1";

function normalizeIncomeBankKey(value) {
  return String(value || "").trim().replace(/\s+/g, " ").toLocaleLowerCase("en-IN");
}

function normalizeIncomeBanks(rawBanks, ledger = []) {
  const names = [
    ...(Array.isArray(rawBanks) ? rawBanks : []),
    ...(Array.isArray(ledger) ? ledger : [])
      .filter((entry) => entry?.kind === "interest" || (entry?.kind === "spend" && entry?.scope === "bank"))
      .map((entry) => entry.bankName),
  ];
  const unique = new Map();
  names.forEach((name) => {
    const displayName = String(name || "").trim().replace(/\s+/g, " ");
    const key = normalizeIncomeBankKey(displayName);
    if (key && !unique.has(key)) unique.set(key, displayName);
  });
  return [...unique.values()].sort((a, b) => a.localeCompare(b, "en-IN"));
}

function normalizeIncomeLedgerEntry(entry = {}) {
  const kind = ["interest", "misc", "spend"].includes(entry.kind) ? entry.kind : "";
  const amount = Math.abs(toNumber(entry.amount));
  if (!kind || amount <= 0) return null;

  const year = kind === "interest" ? Math.round(toNumber(entry.year)) : 0;
  const quarter = kind === "interest" && ["Q1", "Q2", "Q3", "Q4"].includes(entry.quarter)
    ? entry.quarter
    : "";
  const frequency = kind === "interest" && ["monthly", "quarterly"].includes(entry.frequency)
    ? entry.frequency
    : kind === "interest" ? "quarterly" : "";

  return {
    id: String(entry.id || createId()),
    kind,
    bankName: String(entry.bankName || "").trim().replace(/\s+/g, " "),
    source: String(entry.source || "").trim(),
    scope: kind === "spend" && entry.scope === "bank" ? "bank" : "overall",
    frequency,
    year,
    quarter,
    amount,
...
function getInterestLedgerSeedEntries() {
  const interest = [
    ...
    ["IDFC", 2026, "Q2", 1107],
    ["IDFC", 2026, "Q3", 966],
  ].map(([bankName, year, quarter, amount], index) => normalizeIncomeLedgerEntry({
    id: `income-seed-interest-${index + 1}`,
    kind: "interest",
    bankName,
    year,
    quarter,
    frequency: "quarterly",
    amount,
    description: "Imported from Accounts workbook",
    imported: true,
    createdAt: `${year}-${quarter === "Q1" ? "03-31" : quarter === "Q2" ? "06-30" : quarter === "Q3" ? "09-30" : "12-31"}T12:00:00.000Z`,
  }));

  const adjustments = [
    { id: "income-seed-misc-1", kind: "misc", source: "HDFC Sweep in FD", amount: 2128 },
    { id: "income-seed-misc-2", kind: "misc", source: "AMEX Excess cash", amount: 1395 },
    { id: "income-seed-misc-3", kind: "misc", source: "Standard Chartered", amount: 6000 },
    { id: "income-seed-spend-1", kind: "spend", scope: "overall", amount: 9141.35, description: "Aviation Tag" },
    { id: "income-seed-spend-2", kind: "spend", scope: "overall", amount: 4107, description: "Aviation Tag" },
  ].map((entry, index) => normalizeIncomeLedgerEntry({
    ...entry,
    imported: true,
    createdAt: `2026-09-0${index + 1}T12:00:00.000Z`,
  }));

  return [...interest, ...adjustments].filter(Boolean);
}

function ensureInterestLedgerSeed() {
  if (state.interestLedgerSeedVersion === INTEREST_LEDGER_SEED_VERSION) return false;

  const existingIds = new Set(state.interestLedger.map((entry) => entry.id));
  getInterestLedgerSeedEntries().forEach((entry) => {
    if (!existingIds.has(entry.id)) state.interestLedger.push(entry);
  });
  state.incomeBanks = normalizeIncomeBanks(
    [...state.incomeBanks, "HDFC", "DBS", "TMB", "IDFC", "BOB"],
    state.interestLedger
  );
  state.interestLedgerSeedVersion = INTEREST_LEDGER_SEED_VERSION;
  return true;
}
...
async function loadState() {
  try {
    ...
    if (snap.exists()) {
      ...
      const seededInterestLedger = ensureInterestLedgerSeed();
      syncLoungeBenefitsFromVisits();
      if (!state.activityLog.length) {
        state.activityLog = buildInitialActivityLog();
      }
      activitySnapshot = createActivitySnapshot();
      if (migratedLegacyRp || removedDuplicateLegacyRp || migratedWelcomePoints || seededInterestLedger) {
        await saveState({ skipActivity: true });
      }

      console.log("✅ Data loaded from Firebase", state.cards);
    } else {
      console.log("⚠️ No data found in Firebase");
      ensureInterestLedgerSeed();
      activitySnapshot = createActivitySnapshot();
      await saveState({ skipActivity: true });
    }
  } catch (e) {
...
const widgetActivityMeta = {
  portfolio: { title: "Card Portfolio", empty: "No card updates recorded yet." },
  swipes: { title: "Swipes", empty: "No swipe updates recorded yet." },
  rpSpends: { title: "Reward Points", empty: "No reward-point updates recorded yet." },
  ppr: { title: "Partner Programs", empty: "No partner-program updates recorded yet." },
...
function normalizeActivityEntry(entry = {}) {
  const widget = String(entry.widget || "").trim();
  const message = String(entry.message || "").trim();
  if (!widgetActivityMeta[widget] || !message) return null;

  return {
    id: entry.id || createId(),
    widget,
    action: ["added", "updated", "deleted"].includes(entry.action) ? entry.action : "updated",
    message,
    createdAt: entry.createdAt || new Date().toISOString(),
  };
}
...
function createActivityEntry(widget, action, message, createdAt = new Date().toISOString()) {
  return normalizeActivityEntry({ id: createId(), widget, action, message, createdAt });
}

function getActivityCardName(cardId, snapshot) {
  const card = (snapshot?.cards || []).find((item) => item.id === cardId);
  if (!card) return "the selected card";
  return [card.issuer, card.name].filter(Boolean).join(" | ") || "the selected card";
}

function compareActivityRecords(beforeRecords, afterRecords, callback, comparable = (value) => value) {
  const beforeMap = new Map((beforeRecords || []).map((item) => [item.id, item]));
  const afterMap = new Map((afterRecords || []).map((item) => [item.id, item]));
  const ids = new Set([...beforeMap.keys(), ...afterMap.keys()]);

  ids.forEach((id) => {
    const before = beforeMap.get(id);
    const after = afterMap.get(id);
    if (!before && after) callback("added", null, after);
    else if (before && !after) callback("deleted", before, null);
    else if (JSON.stringify(comparable(before)) !== JSON.stringify(comparable(after))) {
      callback("updated", before, after);
    }
  });
}

function getActivityActionVerb(action) {
  if (action === "added") return "Added";
  if (action === "deleted") return "Deleted";
  return "Updated";
}

function activityComparableCard(card = {}) {
  return {
    ...card,
    benefits: (card.benefits || []).filter((benefit) => (
      !String(benefit?.id || "").startsWith(rpRedeemedBenefitPrefix)
      && !String(benefit?.id || "").startsWith("lounge-")
    )),
  };
}

function getActivityCardNormalUnredeemedPoints(snapshot, cardId) {
  const card = (snapshot?.cards || []).find((item) => item.id === cardId);
  if (!card) return 0;

  const redemptions = (snapshot?.rpSpends || [])
    .filter((spend) => !isUnredeemedPointsRecord(spend))
  ...
  return Math.max(0, normalPoints - normalRedeemedPoints);
}

function getActivityRpSpendDescription(action, before, after, snapshot, previousSnapshot = null) {
  ...
  const cardName = getActivityCardName(sourceCardId, snapshot);
  const points = isUnredeemedPointsRecord(spend)
    ? toNumber(spend.points)
    : toNumber(spend.redeemedPoints) || toNumber(spend.points);

  if (isUnredeemedPointsRecord(spend)) {
    const oldValue = previousSnapshot
      ? getActivityCardNormalUnredeemedPoints(previousSnapshot, sourceCardId)
      : 0;
    const newValue = after
      ? getActivityCardNormalUnredeemedPoints(snapshot, sourceCardId)
      : 0;
    return `${verb} unredeemed points for ${cardName}: ${formatPoints(oldValue)} → ${formatPoints(newValue)}.`;
  }

  if (isPartnerProgramRpSpend(spend)) {
    const partnerName = String(spend.partnerName || spend.purchasedFrom || "partner program").trim() || "partner program";
    return `${verb} ${formatPoints(points)} transferred from ${cardName} to ${partnerName}.`;
  }

  const purpose = String(spend.productName || spend.purchasedFrom || "reward redemption").trim();
  return `${verb} ${formatPoints(points)} redemption from ${cardName} for ${purpose}.`;
}

function getActivityPartnerUnredeemedPoints(snapshot, partnerName) {
  const normalizedPartner = normalizePprPartnerName(partnerName);
  if (!normalizedPartner) return 0;

  const cardContributions = (snapshot?.rpSpends || []).reduce((sum, spend) => {
    if (!isPartnerProgramRpSpend(spend)) return sum;
    const spendPartner = String(spend.partnerName || spend.purchasedFrom || "Partner").trim() || "Partner";
    if (normalizePprPartnerName(spendPartner) !== normalizedPartner) return sum;
    ...
    const redeemedPoints = explicitlyRedeemed > 0
      ...
      : toNumber(spend.pointsValue) > 0
        ? sourcePoints
        : 0;
    return sum + Math.max(0, sourcePoints - redeemedPoints);
  }, 0);

  const manualContributions = (snapshot?.pprManualPoints || []).reduce((sum, entry) => {
    if (normalizePprPartnerName(entry.partnerName) !== normalizedPartner) return sum;
    const points = Math.max(0, toNumber(entry.points));
    const redeemedPoints = Math.min(points, Math.max(0, toNumber(entry.redeemedPoints)));
    return sum + Math.max(0, points - redeemedPoints);
  }, 0);

  return cardContributions + manualContributions;
}

function getActivityPartnerBalanceMessage(action, partnerName, before, after) {
  const oldValue = getActivityPartnerUnredeemedPoints(before, partnerName);
  const newValue = getActivityPartnerUnredeemedPoints(after, partnerName);
  const verb = action === "added"
    ? "Added points to"
    : action === "deleted"
      ? "Deleted points from"
      : "Updated";
  return `${verb} ${partnerName}: ${formatPoints(oldValue)} → ${formatPoints(newValue)}.`;
}

function collectWidgetActivityChanges(before, after) {
  const activities = [];
  const seen = new Set();
  const push = (widget, action, message) => {
    const key = `${widget}|${action}|${message}`;
    if (seen.has(key)) return;
    seen.add(key);
    const entry = createActivityEntry(widget, action, message);
    if (entry) activities.push(entry);
  };

  compareActivityRecords(before.cards, after.cards, (action, previous, current) => {
    const card = current || previous || {};
    const name = [card.issuer, card.name].filter(Boolean).join(" | ") || "card";
    let message = `${getActivityActionVerb(action)} card ${name}.`;
    if (action === "updated" && previous?.isGreyedOut !== current?.isGreyedOut) {
      message = `${current?.isGreyedOut ? "Muted" : "Restored"} card ${name}.`;
    } else if (action === "updated" && JSON.stringify(previous?.benefits || []) !== JSON.stringify(current?.benefits || [])) {
      message = `Updated benefits and points for ${name}.`;
    }
    push("portfolio", action, message);
  }, activityComparableCard);

  compareActivityRecords(before.swipes, after.swipes, (action, previous, current) => {
    const swipe = current || previous || {};
    const cardName = getActivityCardName(swipe.cardId, after);
    push("swipes", action, `${getActivityActionVerb(action)} ${formatMoney(swipe.amount)} ${swipe.category || "card"} swipe on ${cardName}.`);
  });
  ...
  compareActivityRecords(before.rpSpends, after.rpSpends, (action, previous, current) => {
    ...
    if (sourceCardId && state.cards.some((card) => card.id === sourceCardId)) {
      push("portfolio", action, description);
    }

    if (isPartnerProgramRpSpend(spend)) {
      const partnerName = String(spend.partnerName || spend.purchasedFrom || "partner program").trim() || "partner program";
      push("ppr", action, getActivityPartnerBalanceMessage(action, partnerName, before, after));
    }
  });

  compareActivityRecords(before.pprManualPoints, after.pprManualPoints, (action, previous, current) => {
    const entry = current || previous || {};
    const partnerName = entry.partnerName || "partner program";
    const oldValue = getActivityPartnerUnredeemedPoints(before, partnerName);
    const newValue = getActivityPartnerUnredeemedPoints(after, partnerName);
    const message = oldValue !== newValue
  ...
  compareActivityRecords(before.loungeVisits, after.loungeVisits, (action, previous, current) => {
    const visit = current || previous || {};
    const cardName = getActivityCardName(visit.cardId, after);
    const location = visit.airport || visit.location || visit.type || "lounge benefit";
    push("lounge", action, `${getActivityActionVerb(action)} ${location} visit for ${cardName}.`);
  });

  compareActivityRecords(before.loungeCardLimits, after.loungeCardLimits, (action, previous, current) => {
    const limit = current || previous || {};
    push("lounge", action, `${getActivityActionVerb(action)} lounge limits for ${getActivityCardName(limit.cardId, after)}.`);
  });

  compareActivityRecords(before.intlTravelTrips, after.intlTravelTrips, (action, previous, current) => {
    const trip = current || previous || {};
    const destination = trip.destination || trip.name || "international trip";
    if (action !== "updated") {
      push("intlTravel", action, `${getActivityActionVerb(action)} ${destination}.`);
      return;
    }

    const beforeExpenses = previous?.expenses || [];
    const afterExpenses = current?.expenses || [];
    let detailLogged = false;
    compareActivityRecords(beforeExpenses, afterExpenses, (expenseAction, oldExpense, newExpense) => {
      const expense = newExpense || oldExpense || {};
      const label = expense.description || expense.category || "expense";
      push("intlTravel", expenseAction, `${getActivityActionVerb(expenseAction)} ${label} in ${destination}.`);
      detailLogged = true;
    });
    compareActivityRecords(previous?.conversions || [], current?.conversions || [], (conversionAction) => {
      push("intlTravel", conversionAction, `${getActivityActionVerb(conversionAction)} currency conversion for ${destination}.`);
      detailLogged = true;
    });
    if (!detailLogged) push("intlTravel", "updated", `Updated trip details for ${destination}.`);
  });

  compareActivityRecords(before.interestLedger, after.interestLedger, (action, previous, current) => {
    const entry = current || previous || {};
    const kindLabel = entry.kind === "interest"
      ? "interest"
      : entry.kind === "misc"
        ? "miscellaneous income"
        : "spend";
    const subject = entry.kind === "interest"
      ? `${entry.bankName || "Bank"} ${entry.quarter || ""} ${entry.year || ""}`.trim()
      : entry.kind === "misc"
        ? entry.source || entry.description || "Miscellaneous income"
        : entry.description || entry.bankName || "Overall spend";
    let message = `${getActivityActionVerb(action)} ${kindLabel} for ${subject}: ${formatMoney(entry.amount)}.`;
    if (action === "updated" && previous && current && toNumber(previous.amount) !== toNumber(current.amount)) {
      message = `Updated ${kindLabel} for ${subject}: ${formatMoney(previous.amount)} → ${formatMoney(current.amount)}.`;
    }
    push("interestIncome", action, message);
  });

  if (activities.length > 40) {
    const affectedWidgets = [...new Set(activities.map((entry) => entry.widget))];
    return affectedWidgets.map((widget) => createActivityEntry(widget, "updated", `Imported or refreshed ${widgetActivityMeta[widget].title} records.`));
  }
...
function buildInitialActivityLog() {
  const entries = [];
  const push = (widget, action, message, createdAt) => {
    if (!createdAt) return;
    const entry = createActivityEntry(widget, action, message, createdAt);
    if (entry) entries.push(entry);
  };
  const snapshot = createActivitySnapshot();

  state.swipes.forEach((swipe) => {
    push("swipes", "added", `Added ${formatMoney(swipe.amount)} ${swipe.category || "card"} swipe on ${getActivityCardName(swipe.cardId, snapshot)}.`, swipe.createdAt);
  });
  ...
  state.rpSpends.forEach((spend) => {
    ...
    if (isPartnerProgramRpSpend(spend)) {
      const partnerName = String(spend.partnerName || spend.purchasedFrom || "partner program").trim() || "partner program";
      push("ppr", "added", `Added partner activity for ${partnerName}.`, spend.redeemedAt || spend.createdAt);
    }
  });
  ...
  state.loungeVisits.forEach((visit) => {
    push("lounge", "added", `Added ${visit.airport || visit.location || visit.type || "lounge benefit"} visit for ${getActivityCardName(visit.cardId, snapshot)}.`, visit.createdAt || visit.date);
  });

  state.loungeCardLimits.forEach((limit) => {
    push("lounge", "added", `Added lounge limits for ${getActivityCardName(limit.cardId, snapshot)}.`, limit.updatedAt || limit.createdAt);
  });

  state.intlTravelTrips.forEach((trip) => {
    (trip.expenses || []).forEach((expense) => {
      push("intlTravel", "added", `Added ${expense.description || expense.category || "expense"} in ${trip.destination || trip.name || "international trip"}.`, expense.createdAt || expense.date || trip.createdAt);
    });
    (trip.conversions || []).forEach((conversion) => {
      push("intlTravel", "added", `Added currency conversion for ${trip.destination || trip.name || "international trip"}.`, conversion.createdAt || conversion.date);
    });
  });

  state.interestLedger.forEach((entry) => {
    const label = entry.kind === "interest"
      ? `${entry.bankName} ${entry.quarter} ${entry.year}`
      : entry.kind === "misc"
        ? entry.source || "Miscellaneous income"
        : entry.description || entry.bankName || "Overall spend";
    const kindLabel = entry.kind === "interest" ? "interest" : entry.kind === "misc" ? "miscellaneous income" : "spend";
    push("interestIncome", "added", `Added ${kindLabel} for ${label}: ${formatMoney(entry.amount)}.`, entry.updatedAt || entry.createdAt);
  });
...
function updateWidgetActivityButton(view = state.currentView) {
  if (!els.widgetActivityBtn) return;
  const normalizedView = normalizeViewName(view);
  const meta = widgetActivityMeta[normalizedView];
  els.widgetActivityBtn.style.display = meta ? "inline-flex" : "none";
  els.widgetActivityBtn.title = meta ? `${meta.title}: recent updates` : "Recent updates";
  els.widgetActivityBtn.setAttribute("aria-label", meta ? `View recent ${meta.title} updates` : "View recent updates");
}
...
function renderWidgetActivityList(view = state.currentView) {
  if (!els.widgetActivityList) return;
  const normalizedView = normalizeViewName(view);
  const meta = widgetActivityMeta[normalizedView];
  ...
  if (!activities.length) {
    els.widgetActivityList.innerHTML = `<div class="widget-activity-empty">${escapeHtml(meta?.empty || "No updates recorded yet.")}</div>`;
    return;
  }

  const actionLabels = { added: "Added", updated: "Updated", deleted: "Deleted" };
  const actionIcons = { added: "+", updated: "↻", deleted: "−" };
  els.widgetActivityList.innerHTML = activities.map((entry) => `
    <article class="widget-activity-item" data-action="${escapeAttribute(entry.action)}">
      <span class="widget-activity-item-icon" aria-hidden="true">${actionIcons[entry.action] || "↻"}</span>
      <div class="widget-activity-copy">
        <strong>${escapeHtml(entry.message)}</strong>
        <span>${escapeHtml(actionLabels[entry.action] || "Updated")}</span>
      </div>
      <time class="widget-activity-time" datetime="${escapeAttribute(entry.createdAt)}">${escapeHtml(formatActivityDateTime(entry.createdAt))}</time>
    </article>
  `).join("");
}

function openWidgetActivityModal() {
  const normalizedView = normalizeViewName(state.currentView);
  const meta = widgetActivityMeta[normalizedView];
  if (!meta || !els.widgetActivityModal) return;

  if (els.widgetActivityTitle) els.widgetActivityTitle.textContent = `${meta.title} Updates`;
  if (els.widgetActivitySubtitle) els.widgetActivitySubtitle.textContent = "Latest 20 changes recorded in this widget.";
  renderWidgetActivityList(normalizedView);
  els.widgetActivityModal.style.display = "flex";
  requestAnimationFrame(() => els.closeWidgetActivityBtn?.focus());
}

function closeWidgetActivityModal() {
  if (!els.widgetActivityModal) return;
  els.widgetActivityModal.style.display = "none";
  els.widgetActivityBtn?.focus();
}

function handleWidgetActivityKeydown(event) {
  if (event.key === "Escape" && els.widgetActivityModal?.style.display === "flex") {
    closeWidgetActivityModal();
  }
}
...
function normalizeRpSpend(spend) {
  ...
  const wholePartnerPointsReceived = isPartnerRecord && parsedRatio && normalizedPointsReceived > 0
    ? Math.round(normalizedPointsReceived)
  ...
  const autoPointsReceived = hasPointsReceivedField
    ? wholePartnerPointsReceived
    : wholePartnerPointsReceived > 0
      ? wholePartnerPointsReceived
...
function computePartnerTransferPoints(points, ratio) {
  ...
  // Partner programs credit whole points. Keep every ratio-derived value
  // consistent across RP Spends, PPR summaries, AI entry, and edit flows.
  return Math.round((pointValue * ratio.to) / ratio.from);
}
...
function handleDashboardStudioAction(event) {
  ...
  if (focusTarget?.dataset.dashboardFocus === "ai") {
    ...
    requestAnimationFrame(() => {
      els.aiCommandInput?.scrollIntoView({ behavior: shouldReduceMotion() ? "auto" : "smooth", block: "center" });
      els.aiCommandInput?.focus();
    });
  }
}
...
function renderDashboard() {
  ...
  const incomeTotals = getIncomeLedgerTotals();
  ...
  if (els.dashboardIncomeNet) {
    els.dashboardIncomeNet.textContent = formatMoney(incomeTotals.net);
    els.dashboardIncomeNet.dataset.valueState = incomeTotals.net < 0 ? "loss" : "profit";
  }
  if (els.dashboardIncomeInterest) els.dashboardIncomeInterest.textContent = formatMoney(incomeTotals.interest);
  if (els.dashboardIncomeMisc) els.dashboardIncomeMisc.textContent = formatMoney(incomeTotals.misc);
...
function resetPortfolioFilters(shouldRender = true) {
  ...
  if (els.sortSelect) els.sortSelect.value = "netDesc";
...
async function saveRpSpendFromForm(event) {
  ...
  // Always validate the final saved debit, including partner-program edits.
  // The Partner Details popup performs an earlier convenience check, but the
  // RP Spend fields can still be changed afterwards. Validating the incremental
  // debit here prevents an existing 30,000-point transfer from being edited to
  // 31,000 when the source card has no additional 1,000 points available.
...
function renderLoungeVisits() {
  ...
  visitsToRender.forEach((visit) => {
      ...
      row.innerHTML = `
        ...
        <div class="money-cell lounge-notes-cell">
          <span class="cell-label">Notes</span>
          <span class="lounge-notes-text${visit.notes ? " has-notes" : ""}">
            ${escapeHtml(visit.notes || "No notes")}
          </span>
        </div>
...
function getIncomeLedgerTotals() {
  const totals = state.interestLedger.reduce((result, entry) => {
    if (entry.kind === "interest") result.interest += toNumber(entry.amount);
    if (entry.kind === "misc") result.misc += toNumber(entry.amount);
    if (entry.kind === "spend") result.spends += toNumber(entry.amount);
    return result;
  }, { interest: 0, misc: 0, spends: 0 });
  totals.net = totals.interest + totals.misc - totals.spends;
  totals.yearCount = new Set(
    state.interestLedger.filter((entry) => entry.kind === "interest" && entry.year).map((entry) => entry.year)
  ).size;
  return totals;
}

function renderIncomeBankOptions(preserveSelection = true) {
  const currentBank = preserveSelection ? els.incomeBankSelect?.value : "";
  const currentSpendBank = preserveSelection ? els.incomeSpendBank?.value : "";
  state.incomeBanks = normalizeIncomeBanks(state.incomeBanks, state.interestLedger);
  const options = state.incomeBanks
    .map((bank) => `<option value="${escapeAttribute(bank)}">${escapeHtml(bank)}</option>`)
    .join("");

  if (els.incomeBankSelect) {
    els.incomeBankSelect.innerHTML = `
      <option value="">Select bank</option>
      ${options}
      <option value="__new__">+ Add new bank</option>
    `;
    if ([...els.incomeBankSelect.options].some((option) => option.value === currentBank)) {
      els.incomeBankSelect.value = currentBank;
    }
  }

  if (els.incomeSpendBank) {
    els.incomeSpendBank.innerHTML = `<option value="">Select bank</option>${options}`;
    if ([...els.incomeSpendBank.options].some((option) => option.value === currentSpendBank)) {
      els.incomeSpendBank.value = currentSpendBank;
    }
  }
}

function updateIncomeEntryFields() {
  const kind = els.incomeEntryType?.value || "interest";
  document.querySelectorAll("#interestIncomeView .income-interest-field").forEach((field) => {
    field.hidden = kind !== "interest";
  });
  document.querySelectorAll("#interestIncomeView .income-misc-field").forEach((field) => {
    field.hidden = kind !== "misc";
  });
  document.querySelectorAll("#interestIncomeView .income-spend-field").forEach((field) => {
    field.hidden = kind !== "spend";
  });

  if (els.incomeNewBankField) {
    els.incomeNewBankField.hidden = kind !== "interest" || els.incomeBankSelect?.value !== "__new__";
  }
  if (els.incomeSpendBankField) {
    els.incomeSpendBankField.hidden = kind !== "spend" || els.incomeSpendScope?.value !== "bank";
  }
  if (els.incomeDescription) {
    els.incomeDescription.placeholder = kind === "spend"
      ? "What was this amount spent on?"
      : kind === "misc"
        ? "Optional details"
        : "Optional interest description";
  }
}

function resetIncomeEntryForm() {
  els.incomeEntryForm?.reset();
  if (els.incomeEditingId) els.incomeEditingId.value = "";
  if (els.incomeEditingBadge) els.incomeEditingBadge.hidden = true;
  if (els.incomeEntryType) els.incomeEntryType.value = "interest";
  if (els.incomeFrequency) els.incomeFrequency.value = "monthly";
  if (els.incomeYear) els.incomeYear.value = String(new Date().getFullYear());
  if (els.incomeQuarter) els.incomeQuarter.value = `Q${Math.floor(new Date().getMonth() / 3) + 1}`;
  if (els.incomeSpendScope) els.incomeSpendScope.value = "overall";
  if (els.saveIncomeEntryBtn) els.saveIncomeEntryBtn.textContent = "Add Entry";
  renderIncomeBankOptions(false);
  updateIncomeEntryFields();
  refreshAllFieldStates();
}

async function saveIncomeEntryFromForm(event) {
  event?.preventDefault();
  const kind = els.incomeEntryType?.value || "interest";
  const amount = toNumber(els.incomeAmount?.value);
  if (amount <= 0) {
  ...
  let source = "";
  let scope = "overall";
  let shouldAddBank = false;
  if (kind === "interest") {
    const selectedBank = els.incomeBankSelect?.value || "";
    if (selectedBank === "__new__") {
      bankName = String(els.incomeNewBankName?.value || "").trim().replace(/\s+/g, " ");
      if (!bankName) {
        showToast("Enter the new bank name.");
        els.incomeNewBankName?.focus();
        return;
      }
      if (state.incomeBanks.some((bank) => normalizeIncomeBankKey(bank) === normalizeIncomeBankKey(bankName))) {
        showToast(`${bankName} already exists. Select it from the bank dropdown.`);
        els.incomeNewBankName?.focus();
        return;
      }
      shouldAddBank = true;
    ...
    const year = Math.round(toNumber(els.incomeYear?.value));
    if (!bankName) {
      showToast("Select a bank or add a new one.");
      els.incomeBankSelect?.focus();
      return;
    }
    if (year < 2000 || year > 2100) {
      showToast("Enter a valid calendar year.");
      els.incomeYear?.focus();
      return;
    }
  } else if (kind === "misc") {
    source = String(els.incomeMiscSource?.value || "").trim();
    if (!source) {
      showToast("Enter the miscellaneous income source.");
      els.incomeMiscSource?.focus();
      return;
    }
  } else {
    scope = els.incomeSpendScope?.value === "bank" ? "bank" : "overall";
    bankName = scope === "bank" ? String(els.incomeSpendBank?.value || "") : "";
    if (scope === "bank" && !bankName) {
      showToast("Select the bank for this deduction.");
      els.incomeSpendBank?.focus();
      return;
    }
  }

  const editingId = els.incomeEditingId?.value || "";
  const previous = state.interestLedger.find((entry) => entry.id === editingId);
  const now = new Date().toISOString();
  const entry = normalizeIncomeLedgerEntry({
    id: editingId || createId(),
    kind,
    bankName,
    source,
    scope,
    frequency: kind === "interest" ? els.incomeFrequency?.value : "",
    year: kind === "interest" ? els.incomeYear?.value : 0,
    quarter: kind === "interest" ? els.incomeQuarter?.value : "",
    amount,
  ...
  if (!entry) {
    showToast("Could not save this entry. Check the entered values.");
    return;
  }

  if (shouldAddBank) state.incomeBanks.push(bankName);

  if (editingId) {
    state.interestLedger = state.interestLedger.map((item) => item.id === editingId ? entry : item);
  } else {
    state.interestLedger.push(entry);
  }
  state.incomeBanks = normalizeIncomeBanks(state.incomeBanks, state.interestLedger);
  await saveState();
  resetIncomeEntryForm();
  render();
  showToast(editingId ? "Income entry updated." : "Income entry added.");
}

function editIncomeLedgerEntry(entryId) {
  const entry = state.interestLedger.find((item) => item.id === entryId);
  if (!entry) return;
  renderIncomeBankOptions(false);
  if (els.incomeEditingId) els.incomeEditingId.value = entry.id;
  if (els.incomeEditingBadge) els.incomeEditingBadge.hidden = false;
  if (els.incomeEntryType) els.incomeEntryType.value = entry.kind;
  if (els.incomeFrequency) els.incomeFrequency.value = entry.frequency || "monthly";
  if (els.incomeYear) els.incomeYear.value = entry.year || new Date().getFullYear();
  if (els.incomeQuarter) els.incomeQuarter.value = entry.quarter || "Q1";
  if (els.incomeBankSelect && entry.kind === "interest") els.incomeBankSelect.value = entry.bankName;
  if (els.incomeMiscSource) els.incomeMiscSource.value = entry.source || "";
  if (els.incomeSpendScope) els.incomeSpendScope.value = entry.scope || "overall";
  if (els.incomeSpendBank) els.incomeSpendBank.value = entry.bankName || "";
  if (els.incomeAmount) els.incomeAmount.value = entry.amount;
  if (els.incomeDescription) els.incomeDescription.value = entry.description || "";
  if (els.incomeNotes) els.incomeNotes.value = entry.notes || "";
  if (els.saveIncomeEntryBtn) els.saveIncomeEntryBtn.textContent = "Save Changes";
  updateIncomeEntryFields();
  refreshAllFieldStates();
  els.incomeEntryForm?.scrollIntoView({ behavior: shouldReduceMotion() ? "auto" : "smooth", block: "start" });
  window.setTimeout(() => els.incomeAmount?.focus(), shouldReduceMotion() ? 0 : 320);
}

async function deleteIncomeLedgerEntry(entryId) {
  const entry = state.interestLedger.find((item) => item.id === entryId);
  if (!entry) return;
  const label = entry.kind === "interest" ? `${entry.bankName} ${entry.quarter} ${entry.year}` : entry.source || entry.description || "this entry";
  if (!window.confirm(`Delete ${label}? The totals will be recalculated.`)) return;
  state.interestLedger = state.interestLedger.filter((item) => item.id !== entryId);
  if (els.incomeEditingId?.value === entryId) resetIncomeEntryForm();
  await saveState();
  render();
  showToast("Income entry deleted and totals recalculated.");
}

function handleIncomeLedgerAction(event) {
  const button = event.target.closest?.("[data-income-action]");
  if (!button) return;
  const entryId = button.dataset.entryId;
  if (button.dataset.incomeAction === "edit") editIncomeLedgerEntry(entryId);
  if (button.dataset.incomeAction === "delete") deleteIncomeLedgerEntry(entryId);
}

function renderIncomeEntryActions(entry) {
  return `
    <div class="income-record-actions">
      <button class="icon-button" type="button" data-income-action="edit" data-entry-id="${escapeHtml(entry.id)}" title="Edit entry" aria-label="Edit entry">
        <svg viewBox="0 0 24 24" aria-hidden="true"><path d="m4 20 4.5-1 10-10a2.1 2.1 0 0 0-3-3l-10 10L4 20Z" /></svg>
      </button>
      <button class="icon-button danger" type="button" data-income-action="delete" data-entry-id="${escapeHtml(entry.id)}" title="Delete entry" aria-label="Delete entry">
...
function renderInterestQuarter(year, quarter, entries) {
  const total = entries.reduce((sum, entry) => sum + toNumber(entry.amount), 0);
  const rows = entries.length
    ? entries.map((entry) => `
        <div class="income-record-row">
          <div class="income-record-main">
            <strong>${escapeHtml(entry.bankName)}</strong>
            <span>${escapeHtml(entry.frequency === "monthly" ? "Monthly" : "Quarterly")}${entry.description && entry.description !== "Imported from Accounts workbook" ? ` · ${escapeHtml(entry.description)}` : ""}</span>
            ${entry.notes ? `<small>${escapeHtml(entry.notes)}</small>` : ""}
          </div>
          <strong class="income-record-amount">${escapeHtml(formatMoney(entry.amount))}</strong>
          ${renderIncomeEntryActions(entry)}
        </div>
      `).join("")
    : `<div class="income-quarter-empty">No interest entries</div>`;

  return `
    <section class="income-quarter-card">
      <header><div><span>${quarter}</span><small>${year}</small></div><strong>${escapeHtml(formatMoney(total))}</strong></header>
      <div class="income-quarter-records">${rows}</div>
    </section>
  `;
}

function renderIncomeLifetimeBlock(kind, title, entries) {
  const total = entries.reduce((sum, entry) => sum + toNumber(entry.amount), 0);
  const rows = entries.length
    ? entries.map((entry) => {
        const heading = kind === "misc"
          ? entry.source || "Miscellaneous income"
          : entry.description || (entry.scope === "bank" ? entry.bankName : "Overall deduction");
        const detail = kind === "spend"
          ? entry.scope === "bank" ? `Bank-specific · ${entry.bankName}` : "Overall amount"
          : "Additional income";
        return `
          <div class="income-lifetime-row">
            <div><strong>${escapeHtml(heading)}</strong><span>${escapeHtml(detail)}</span>${entry.notes ? `<small>${escapeHtml(entry.notes)}</small>` : ""}</div>
            <strong class="income-record-amount ${kind === "spend" ? "is-spend" : ""}">${kind === "spend" ? "−" : "+"}${escapeHtml(formatMoney(entry.amount))}</strong>
            ${renderIncomeEntryActions(entry)}
          </div>
        `;
      }).join("")
    : `<div class="income-quarter-empty">No ${kind === "misc" ? "miscellaneous income" : "spends"} recorded</div>`;

  return `
    <details class="income-lifetime-block" open>
      <summary><div><span>${escapeHtml(title)}</span><small>${entries.length} ${entries.length === 1 ? "entry" : "entries"}</small></div><strong>${kind === "spend" ? "−" : "+"}${escapeHtml(formatMoney(total))}</strong></summary>
      <div class="income-lifetime-records">${rows}</div>
    </details>
  `;
}

function ensureIncomeBankTotalsElement() {
  let totalsElement = document.getElementById("incomeBankTotals");
  if (totalsElement) {
    els.incomeBankTotals = totalsElement;
    return totalsElement;
  }

  const form = els.incomeEntryForm;
  const actions = form?.querySelector(".income-entry-actions");
  if (!form || !actions) return null;

  let footer = actions.closest(".income-entry-footer");
  if (!footer) {
    footer = document.createElement("div");
    footer.className = "income-entry-footer";
    actions.parentElement?.insertBefore(footer, actions);
    footer.appendChild(actions);
  }

  const summary = document.createElement("section");
  summary.className = "income-bank-contribution-summary";
  summary.setAttribute("aria-labelledby", "incomeBankTotalsTitle");
  summary.innerHTML = `
    <div class="income-bank-contribution-heading">
      <div>
        <span id="incomeBankTotalsTitle">Interest Contribution by Bank</span>
        <small>Monthly and quarterly interest combined. Deductions apply only to the overall balance.</small>
      </div>
    </div>
    <div id="incomeBankTotals" class="income-bank-total-list" aria-live="polite"></div>
  `;
  footer.insertBefore(summary, actions);
  totalsElement = summary.querySelector("#incomeBankTotals");
  els.incomeBankTotals = totalsElement;
  return totalsElement;
}

function renderInterestIncomeWidget() {
  if (!els.incomeLedgerBlocks) return;
  ensureIncomeBankTotalsElement();
  renderIncomeBankOptions(true);
  updateIncomeEntryFields();
  const totals = getIncomeLedgerTotals();
  if (els.incomeRenderInterest) els.incomeRenderInterest.textContent = formatMoney(totals.interest);
  if (els.incomeRenderMisc) els.incomeRenderMisc.textContent = formatMoney(totals.misc);
  if (els.incomeRenderSpends) els.incomeRenderSpends.textContent = formatMoney(totals.spends);
  if (els.incomeRenderNet) {
    els.incomeRenderNet.textContent = formatMoney(totals.net);
    els.incomeRenderNet.classList.toggle("is-negative", totals.net < 0);
  }

  const interestEntries = state.interestLedger.filter((entry) => entry.kind === "interest");
  if (els.incomeBankTotals) {
    const bankTotals = new Map();
    interestEntries.forEach((entry) => {
      const key = normalizeIncomeBankKey(entry.bankName);
      if (!key) return;
      const current = bankTotals.get(key) || { name: entry.bankName, total: 0, count: 0 };
      current.total += toNumber(entry.amount);
      current.count += 1;
      bankTotals.set(key, current);
    });
    const sortedBankTotals = [...bankTotals.values()].sort((a, b) => a.name.localeCompare(b.name, "en-IN"));
    els.incomeBankTotals.innerHTML = sortedBankTotals.length
      ? sortedBankTotals.map((bank) => `
          <div class="income-bank-total-item">
            <span>${escapeHtml(bank.name)}</span>
            <strong>${escapeHtml(formatMoney(bank.total))}</strong>
            <small>${bank.count} ${bank.count === 1 ? "interest entry" : "interest entries"}</small>
          </div>
        `).join("")
      : `<div class="income-bank-total-empty">No bank interest recorded yet.</div>`;
  }
  const years = [...new Set(interestEntries.map((entry) => entry.year).filter(Boolean))].sort((a, b) => b - a);
  const yearBlocks = years.map((year, index) => {
    const yearEntries = interestEntries.filter((entry) => entry.year === year);
    const yearTotal = yearEntries.reduce((sum, entry) => sum + toNumber(entry.amount), 0);
    const bankCount = new Set(yearEntries.map((entry) => normalizeIncomeBankKey(entry.bankName))).size;
    const quarterTotals = ["Q1", "Q2", "Q3", "Q4"].map((quarter) => {
      const quarterEntries = yearEntries.filter((entry) => entry.quarter === quarter);
      return {
        quarter,
        total: quarterEntries.reduce((sum, entry) => sum + toNumber(entry.amount), 0),
        count: quarterEntries.length,
      };
    });
    return `
      <details class="income-year-block" ${index === 0 ? "open" : ""}>
        <summary>
          <div class="income-year-summary-grid">
            <div class="income-year-identity">
              <span>Calendar Year</span>
              <strong>${year}</strong>
              <small>${bankCount} ${bankCount === 1 ? "bank" : "banks"} · ${yearEntries.length} entries</small>
            </div>
            ${quarterTotals.map(({ quarter, total, count }) => `
              <div class="income-year-quarter-summary">
                <span>${quarter}</span>
                <strong>${escapeHtml(formatMoney(total))}</strong>
                <small>${count} ${count === 1 ? "entry" : "entries"}</small>
              </div>
            `).join("")}
            <div class="income-year-total-summary">
              <span>Year Total</span>
              <strong>${escapeHtml(formatMoney(yearTotal))}</strong>
            </div>
          </div>
        </summary>
        <div class="income-quarter-grid">
          ${["Q1", "Q2", "Q3", "Q4"].map((quarter) => renderInterestQuarter(
            year,
            quarter,
            yearEntries
              .filter((entry) => entry.quarter === quarter)
              .sort((a, b) => a.bankName.localeCompare(b.bankName, "en-IN") || new Date(a.createdAt) - new Date(b.createdAt))
          )).join("")}
        </div>
      </details>
    `;
  ...
  const miscEntries = state.interestLedger.filter((entry) => entry.kind === "misc");
  const spendEntries = state.interestLedger.filter((entry) => entry.kind === "spend");
  els.incomeLedgerBlocks.innerHTML = `
    ${yearBlocks || `<div class="income-ledger-empty">No interest years recorded yet.</div>`}
    ${renderIncomeLifetimeBlock("misc", "Miscellaneous Income", miscEntries)}
    ${renderIncomeLifetimeBlock("spend", "Spends", spendEntries)}
  `;
}
  ...
  cards.slice(0, visibleCount).forEach((card) => {
    ...
    row.innerHTML = `
      ...
      <div class="money-cell">
        ...
        ${formatBenefitSplitHtml(
          totals.benefits,
          getCardUnredeemedPoints(card),
          getCardPointAllocation(card).redeemedPoints
        )}
      </div>
      <div class="money-cell">
...
function importPortfolio(event) {
  ...
  reader.onload = () => {
    try {
      ...
      ensureInterestLedgerSeed();
      syncLoungeBenefitsFromVisits();
      if (!state.activityLog.length) state.activityLog = buildInitialActivityLog();
      const importedAt = new Date().toISOString();
      state.activityLog = [
        ...Object.keys(widgetActivityMeta).map((widget) => createActivityEntry(widget, "updated", `Imported ${widgetActivityMeta[widget].title} records.`, importedAt)),
        ...state.activityLog,
      ].slice(0, 500);
      activitySnapshot = createActivitySnapshot();
      saveState({ skipActivity: true });
      resetForm();
      render();
...
function getPartnerProgramPoints(rpSpend) {
  ...
  if (receivedPoints > 0) return Math.round(receivedPoints);
  const ratio = parsePartnerTransferRatio(rpSpend?.partnerTransferRatio);
...
function formatBenefitSplitHtml(cash, points, redeemedPoints = 0) {
  const cashLine = `<span class="value-row"><span>Total</span><strong>${escapeHtml(formatMoney(cash))}</strong></span>`;
  const redeemedLine = `<span class="value-row"><span>Redeemed</span><strong>${escapeHtml(formatPoints(redeemedPoints))}</strong></span>`;
  const pointsLine = `<span class="value-row"><span>Unredeemed</span><strong>${escapeHtml(formatPoints(points))}</strong></span>`;
  return `${cashLine}${redeemedLine}${pointsLine}`;
}
...
function formatCardBenefitsHtml(card) {
  ...
  const unredeemedBenefits = visibleBenefits.filter((benefit) => benefit?.type === "Unredeemed Points");
  const otherBenefits = visibleBenefits.filter((benefit) => (
  ...
  const welcomeUnredeemedLine = hasWelcomePoints ? `
    <span class="benefit-line benefit-line-derived">
      <span class="benefit-line-name" style="font-style: italic;">Welcome Benefits Unredeemed</span>
      <span class="benefit-line-meta" style="background: rgba(96, 165, 250, 0.12); padding: 2px 8px; border-radius: 12px; font-style: italic;">Welcome Benefit (Points) | Unredeemed</span>
...
function updateSortColor() {
  if (els.sortSelect) {
    if (els.sortSelect.value !== "netDesc") {
