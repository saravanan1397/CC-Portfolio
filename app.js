...
function renderSummary() {
  ...
  els.benefitHint.textContent = totals.points
    ? `${totals.benefitCount} benefit ${totals.benefitCount === 1 ? "entry" : "entries"} and ${formatPoints(totals.points)}`
    : `${totals.benefitCount} benefit ${totals.benefitCount === 1 ? "entry" : "entries"}`;
...
function renderCards() {
  ...
  cards.forEach((card) => {
    ...
    const topBenefit = getTopBenefit(card);
    const meta = [
      card.issuer,
    ...
    row.innerHTML = `
      <div class="card-name">
        ...
        ${topBenefit ? `<span class="benefit-chip">${escapeHtml(topBenefit.type)}: ${escapeHtml(formatBenefitValue(topBenefit))}</span>` : ""}
      </div>
      ...
      <div class="money-cell">
        ...
        ${formatMixedValueHtml(totals.benefits, totals.points)}
      </div>
      <div class="money-cell">
...
function getTotals(cards) {
  return cards.reduce(
    ...
    { fees: 0, benefits: 0, points: 0, net: 0, surplus: 0, deficit: 0, benefitCount: 0 }
  );
}
...
function getTopBenefit(card) {
  return [...card.benefits].sort((a, b) => toNumber(b.amount) - toNumber(a.amount))[0];
}
