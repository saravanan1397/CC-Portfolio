...
function detectAiAction(normalizedQuery) {
  const hasSearchCue = /\b(tell me|show|search|find|list|display|what(?:'s| is)?|how many|how much|where|which)\b/.test(normalizedQuery);
  ...
  if (hasSearchCue && !/\b(add|log|record|redeem|redeemed|update|edit|change)\b/.test(normalizedQuery)) return "search";
  if (/\bswipe\b/.test(normalizedQuery) && /\d/.test(normalizedQuery)) return "add";
  if (/\blounge\b|\bvisit\b/.test(normalizedQuery) && (/\d+\s*members?\b/.test(normalizedQuery) || /\bworth\b/.test(normalizedQuery) || /\bvalue\b/.test(normalizedQuery))) return "add";
  if (/\b(points|miles)\b/.test(normalizedQuery) && /\d/.test(normalizedQuery) && !/\b(total|my|how many|show|search|fee|emi|fy)\b/.test(normalizedQuery)) return "add";
  return "search";
}
...
function detectAiModule(normalizedQuery, action) {
  ...
  if (/\b(swipe|emi|full swipe|spent for|business|personal)\b/.test(normalizedQuery)) return "swipes";
...
function extractFinancialYear(text) {
  const normalized = normalizeAiText(text);
  const match = normalized.match(/\bfy\s*(\d{2})(?:\s*[-/ ]\s*(\d{2}))?\b/);
  if (!match) return "";
  const start = match[1];
  const end = match[2] || String((Number(start) + 1) % 100).padStart(2, "0");
  return `FY ${start}-${end}`;
}
...
async function saveAiPortfolioRecord(resolved) {
  ...
  const updated = JSON.parse(JSON.stringify(state.cards[existingIndex]));
  updated.benefits = Array.isArray(updated.benefits) ? updated.benefits.slice() : [];
  updated.benefits.push({
    id: createId(),
    type: "Welcome Benefit (Points)",
    valueType: "points",
    label: "AI Added Points",
    amount: toNumber(resolved.points),
  });
  state.cards[existingIndex] = normalizeCard(updated);
  syncLoungeBenefitsFromVisits();
  recordAiTrainingExample(resolved, updated);
  await saveState();
  render();
}
