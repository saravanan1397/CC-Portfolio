...
function getIntlTravelTripTotal(trip) {
  return (trip?.expenses || []).reduce((sum, expense) => sum + toNumber(expense.inrAmount) + toNumber(expense.markupFee), 0);
}
...
function getIntlTravelTotals() {
  ...
  return {
    ...
    expenseInr: expenses.reduce((sum, expense) => sum + toNumber(expense.inrAmount), 0),
    markupInr: expenses.reduce((sum, expense) => sum + toNumber(expense.markupFee), 0),
    totalInr: expenses.reduce((sum, expense) => sum + toNumber(expense.inrAmount) + toNumber(expense.markupFee), 0),
  };
}
...
function getIntlTravelTripCurrencies(trip) {
  return getIntlTravelCurrencyTotals(trip?.expenses || [])
    ...
    .map(({ currency, localAmount }) => `${currency} ${formatIntlTravelAmount(localAmount)}`)
    .join(" | ");
}
...
function getIntlTravelCurrencyTotals(expenses = []) {
  ...
  expenses.forEach((expense) => {
    ...
    current.localAmount += toNumber(expense.localAmount);
    current.inrAmount += toNumber(expense.inrAmount) + toNumber(expense.markupFee);
...
function getIntlTravelPhaseTotals(expenses = []) {
  ...
  return phases.map((phase) => {
    ...
    return {
      ...
      totalInr: phaseExpenses.reduce((sum, expense) => sum + toNumber(expense.inrAmount) + toNumber(expense.markupFee), 0),
      ...
      <div class="intl-travel-currency-card-grid">
        ${currencyTotals.map(({ currency, localAmount, inrAmount }) => `
          <article class="intl-travel-currency-card">
            ...
            <strong>${escapeHtml(`${currency} ${formatIntlTravelAmount(localAmount)}`)}</strong>
        ...
        ${phaseTotals.map((phase) => `
          <article class="intl-travel-phase-card">
            ...
            <em>${escapeHtml(phase.currencies.length
              ? phase.currencies.map(({ currency, localAmount }) => `${currency} ${formatIntlTravelAmount(localAmount)}`).join(" | ")
...
function renderIntlTravelDetail(trip) {
  ...
  const total = filteredExpenses.reduce((sum, expense) => sum + toNumber(expense.inrAmount) + toNumber(expense.markupFee), 0);
  const markupTotal = filteredExpenses.reduce((sum, expense) => sum + toNumber(expense.markupFee), 0);
  const activeFilters = [
  ...
  els.intlTravelExpenseTable.innerHTML = `
    ...
    ${expensesToRender.map((expense) => `
      <article class="intl-travel-expense-row" draggable="true" data-expense-id="${escapeAttribute(expense.id)}">
        ...
        <strong class="intl-travel-cell intl-travel-local-value">${escapeHtml(`${expense.currency} ${formatIntlTravelAmount(expense.localAmount)}`)}</strong>
        <strong class="intl-travel-cell intl-travel-inr-value">${escapeHtml(formatMoney(expense.inrAmount + expense.markupFee))}</strong>
