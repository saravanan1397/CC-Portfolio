...
const benefitValueTypes = [
  { value: "cash", label: "INR / Cashpoints" },
  { value: "points", label: "Reward points / Miles" },
];
...
const sampleCards = [
  {
    ...
    benefits: [
      { id: "b1", type: "Lounge", label: "Airport lounge visits", amount: 8000, valueType: "cash" },
      { id: "b2", type: "Voucher", label: "Hotel voucher", amount: 6000, valueType: "cash" },
      { id: "b3", type: "Rewards", label: "Reward points redeemed", amount: 3500, valueType: "points" },
    ],
  },
  {
    id: "sample-cashback",
    ...
    benefits: [
      { id: "b4", type: "Rewards", label: "Cashback received", amount: 7800, valueType: "cash" },
      { id: "b5", type: "Statement credit", label: "Utility offer", amount: 1200, valueType: "cash" },
    ],
  },
  {
    id: "sample-fuel",
    ...
    benefits: [
      { id: "b6", type: "Statement credit", label: "Fuel surcharge waiver", amount: 2100, valueType: "cash" },
      { id: "b7", type: "Milestone", label: "Quarterly bonus", amount: 750, valueType: "cash" },
    ],
  },
];
...
function cacheElements() {
  Object.assign(els, {
    ...
    renewalMonth: document.getElementById("renewalMonth"),
...
function renderSummary() {
  ...
  els.netHint.textContent = cardCount
    ? totals.net >= 0
      ? `${formatMoney(surplusAfterRecovery)} surplus after covering losses`
      : `${formatMoney(Math.abs(totals.net))} short of break-even`
    : "No cards added";
  els.feeValue.textContent = formatMoney(totals.fees);
  els.feeHint.textContent = `${cardCount} ${cardCount === 1 ? "card" : "cards"} tracked`;
  els.benefitValue.textContent = formatMoney(totals.benefits);
  els.benefitHint.textContent = `${totals.cashBenefitCount} INR-based ${totals.cashBenefitCount === 1 ? "entry" : "entries"} included in P/L`;
  els.pointsValue.textContent = formatPoints(totals.points);
  els.pointsHint.textContent = `${totals.pointBenefitCount} points-based ${totals.pointBenefitCount === 1 ? "entry" : "entries"} not in P/L yet`;
  ...
  els.lossGap.textContent = formatMoney(uncovered);
  els.recoveryBar.style.width = `${coverage}%`;

  if (!cardCount) {
    els.recoveryTitle.textContent = "Portfolio recovery";
    els.recoveryText.textContent = "Add cards to see whether profitable cards cover high-fee cards.";
  } else if (!totals.deficit) {
    els.recoveryTitle.textContent = "All cards are recovered";
    els.recoveryText.textContent = `Every tracked card is at break-even or better. Portfolio net is ${formatMoney(totals.net)}.`;
  } else if (uncovered === 0) {
    els.recoveryTitle.textContent = "Loss cards are covered";
    els.recoveryText.textContent = `${formatMoney(totals.surplus)} surplus covers ${formatMoney(totals.deficit)} in card-level losses.`;
  } else {
    els.recoveryTitle.textContent = "Recovery gap";
...
function renderBenefitsEditor() {
  ...
  draftBenefits.forEach((benefit, index) => {
    ...
    row.innerHTML = `
      ...
      <label class="field">
        <span>Benefit</span>
        <input data-benefit-field="label" type="text" value="${escapeAttribute(benefit.label)}" placeholder="Voucher / points / credit" autocomplete="off" />
      </label>
      <label class="field">
...
function renderCards() {
  ...
  cards.forEach((card) => {
    ...
    const meta = [
      card.issuer,
      card.renewalMonth && `Renews ${card.renewalMonth}`,
      card.targetValue > 0 && `Target ${formatMoney(card.targetValue)}`,
    ]
      .filter(Boolean)
      .join(" | ");
    ...
    row.innerHTML = `
      ...
      <div class="money-cell">
        <span class="cell-label">Benefits</span>
...
function saveCardFromForm(event) {
  ...
  const card = normalizeCard({
    ...
    renewalMonth: els.renewalMonth.value,
...
function populateForm(card) {
  ...
  els.renewalMonth.value = card.renewalMonth;
...
function getStatus(net) {
  ...
  return { key: "breakeven", label: "Break-even" };
}
...
function formatBenefitSplitHtml(cash, points) {
  const cashLine = `<span class="value-row"><span>INR/cashpoints</span><strong>${escapeHtml(formatMoney(cash))}</strong></span>`;
  const pointsLine = `<span class="value-row"><span>Reward points</span><strong>${escapeHtml(formatPoints(points))}</strong></span>`;
  return `${cashLine}${pointsLine}`;
}

function formatCardBenefitsHtml(card) {
  if (!card.benefits.length) {
    return `<div class="benefit-breakdown muted-breakdown">No benefits added</div>`;
  }
  ...
  return `
    <div class="benefit-breakdown">
      ${card.benefits
        .map((benefit) => {
          const valueTypeLabel = isPointBenefit(benefit) ? "Reward points" : "INR/cashpoints";
          const impactLabel = isPointBenefit(benefit) ? "Tracked" : "In P/L";
