...
function showPointsPopup() {
  ...
  const pointCards = state.cards.filter((card) => {
    return card.benefits.some(b => b.type === "Points Redeemed" || isPointBenefit(b));
  }).sort((a, b) => {
    const totalsA = getCardTotals(a);
    const totalsB = getCardTotals(b);
    if (totalsB.points !== totalsA.points) {
  ...
  if (!pointCards.length) {
    ...
    content.innerHTML = pointCards.map((card) => {
      const totals = getCardTotals(card);
      const redeemedPoints = card.benefits
        .filter(b => b.type === "Points Redeemed")
        .reduce((sum, b) => sum + toNumber(b.amount), 0);
      
      const redeemedHtml = redeemedPoints > 0 ? `<div style="font-size:12px; color:#64748b; margin-top:2px;">Redeemed: ${formatPoints(redeemedPoints)}</div>` : "";
  ...
  if (!feeCards.length) {
    ...
    content.innerHTML = feeCards.map((card) => {
      const totals = getCardTotals(card);
        ...
        let feeHtml = "";
        if (currentFee > 0) {
          feeHtml += `<div>Joined Month: ${formatMoney(currentFee)}</div>`;
        }
