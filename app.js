...
function getCardNormalPointsBaseline(card) {
  ...
  if (!manualSources.length) {
    const autoSources = (card.benefits || [])
      .filter((benefit) => benefit?.type === "Unredeemed Points" && isRpRedeemedAutoBenefit(benefit));
    ...
    return autoSources.reduce((sum, benefit) => {
      const storedOriginalPoints = toNumber(benefit.originalPoints);
      const hasStoredBaseline = Object.prototype.hasOwnProperty.call(benefit, "originalPoints");
      return sum + (hasStoredBaseline
        ? storedOriginalPoints
        : toNumber(benefit.amount) + historicalRedemptions);
    }, 0);
  }
...
function syncRpRedeemedBenefitsFromSpends() {
  ...
  state.cards = state.cards.map((card) => {
    ...
    const hasPointAllocation = Boolean(
      allocation.welcomePoints > 0
      || allocation.normalPoints > 0
      || allocation.totalUnredeemedPoints > 0
    );
    const unredeemedPoints = hasPointAllocation
      ? allocation.normalRemainingPoints
      : Math.max(0, cardTotals?.unredeemedPoints || 0);
    const normalRedeemedPoints = hasPointAllocation
      ? allocation.normalRedeemedPoints
      : Math.max(0, cardTotals?.redeemedPoints || 0);
    const normalRedeemedValue = hasPointAllocation
    ...
    if (unredeemedPoints > 0) {
      benefits.push({
        ...
        originalPoints: hasPointAllocation
          ? allocation.normalPoints
          : Math.max(
            0,
            (cardTotals?.unredeemedPoints || 0) + (cardTotals?.redeemedPoints || 0)
          ),
      });
    }
    ...
    if (normalRedeemedValue > 0 || normalRedeemedPoints > 0) {
      benefits.push({
        ...
        valueType: "cash",
        label: hasPointAllocation && allocation.welcomePoints > 0
...
function renderDashboard() {
  ...
  if (els.dashboardNetValue) {
    ...
    els.dashboardNetValue.style.color = totals.net > 0 ? "#10b981" : "#ef4444";
  ...
  if (els.dashboardIntlTravelHint) {
    els.dashboardIntlTravelHint.textContent = intlTravelTotals.tripCount
      ? `${intlTravelTotals.tripCount} ${intlTravelTotals.tripCount === 1 ? "trip" : "trips"}`
      : "No trips recorded";
  }
}
        ...
        <article class="card-row ppr-partner-row" data-partner-name="${escapeAttribute(row.partnerName)}" tabindex="0" role="button" aria-label="View details for ${escapeAttribute(row.partnerName)}" style="padding: 10px 12px; gap: 0; align-items: center; margin: 0;">
          ...
          <div class="money-cell" style="min-width: 42px; padding: 0 6px; justify-content: center;">
            <div class="row-actions inline-actions" style="gap: 4px;">
              <button class="icon-button subtle ppr-edit-value" data-partner-name="${escapeAttribute(row.partnerName)}" data-current-value="${row.value || 0}" data-ppr-scope="${scope}" title="Edit partner value" aria-label="Edit partner value" style="padding: 4px;">
