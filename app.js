...
function toggleFutureFeeFields(show) {
  ...
  if (els.futureFeeDateContainer) {
    els.futureFeeDateContainer.style.display = display;
  }

  if (els.futureAnnualFeeContainer) {
    els.futureAnnualFeeContainer.style.display = display;
  }

  if (els.futureFeeList) {
    els.futureFeeList.style.display = display;
  }
        ...
        ">
          <div>
            <div style="font-weight:700; color:#f8fafc; font-size:15px;">
              ${escapeHtml(card.issuer || "Unknown Bank")} | ${escapeHtml(card.name)}
            </div>
            <div style="font-size:12px; color:#94a3b8; margin-top:4px; line-height:1.4;">
              ${escapeHtml(feeBreakdown)}
            </div>
          </div>
  ...
  cards.forEach((card) => {
    ...
    row.innerHTML = `
      <div class="card-name">
        ...
        ${formatFutureAnnualFeesHtml(card)}
...
function getCardTotals(card) {
  let fees = toNumber(card.annualFee) + toNumber(card.taxFee) + getFutureAnnualFeeTotal(card);

  if (shouldIncludePreviousFee(card)) {
...
function shouldIncludePreviousFee(card) {
  const memberYear = Number(card.memberSince?.split("-")[0]);
  const currentYear = new Date().getFullYear();

  return Boolean(memberYear && memberYear < currentYear);
}
...
function formatFeeBreakdown(card) {
  ...
  const previousFee = toNumber(card.prevYearFee);
  const upcomingFee = getFutureAnnualFeeTotal(card);
  const parts = [];

  if (currentFee > 0) {
    parts.push(`Annual: ${formatMoney(currentFee)}`);
  }

  if (shouldIncludePreviousFee(card) && previousFee > 0) {
    parts.push(`Joining: ${formatMoney(previousFee)}`);
  }
...
function formatFutureAnnualFeesHtml(card) {
  if (!card.futureAnnualFees?.length) {
    return "";
  }

  return `
    <div class="benefit-breakdown">
      ${card.futureAnnualFees
        .map((fee) => `
          <span class="benefit-line">
            <span class="benefit-line-name">Upcoming Annual Fee</span>
            <span class="benefit-line-meta">${escapeHtml(formatMonthYear(fee.month))}</span>
            <strong>${escapeHtml(formatMoney(fee.amount))}</strong>
          </span>
        `)
        .join("")}
    </div>
