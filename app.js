...
function normalizeRpSpend(spend) {
  ...
  const normalizedRedeemedPoints = toNumber(spend.redeemedPoints ?? spend.redeemed ?? spend.redeemedPointsTotal);
  const rawPoints = toNumber(spend.points);
  const hasSplitRedemptionModel = spend.redemptionModel === "split-v2" || String(id).startsWith("legacy-");
...
function syncRpRedeemedBenefitsFromSpends() {
  ...
  state.rpSpends.forEach((rpSpend) => {
    ...
    cardTotals.redeemedValue += toNumber(rpSpend.pointsValue);
...
function updatePartnerProgramOriginInfo(selectEl, infoEl, pendingRedeemedPoints = 0) {
  ...
  const savedUnredeemedPoints = getCardUnredeemedPoints(card);
...
function showPartnerProgramTransferPrompt(initial = {}) {
  return new Promise((resolve) => {
    ...
    const refreshInfo = () => {
      ...
      updatePartnerProgramOriginInfo(originatingCardSelect, infoEl, pendingRedemptionsByCard[cardId] || 0);
    };
    ...
    redeemBtn?.addEventListener("click", async () => {
      ...
      const availablePoints = Math.max(0, getCardUnredeemedPoints(sourceCard) - alreadyPendingPoints);
...
function getCardUnredeemedPoints(card) {
  if (!card || !Array.isArray(card.benefits)) return 0;
...
function applyCardPointRedemption(cardId, redeemPoints) {
  ...
  if (cardIndex < 0) {
    return { ok: false, error: "Selected card was not found." };
  }

  const card = state.cards[cardIndex];
  const currentUnredeemed = getCardUnredeemedPoints(card);
...
async function saveRpSpendFromForm(event) {
  ...
  const rpSpend = normalizeRpSpend({
    ...
    redeemedPoints: els.rpRedeemedPoints?.value,
    redemptionModel: "split-v2",
    pointsValue: els.rpPointsValue?.value,
    redemptionCharges: els.rpRedemptionCharges?.value,
    cardPaid: els.rpCardPaid?.value,
...
function populateRpSpendForm(rpSpend) {
  ...
  if (els.rpPoints) els.rpPoints.value = getRpSpendTotalPoints(rpSpend) || "";
...
function renderRpSpends() {
  ...
  groupsToRender.forEach((group) => {
    ...
    const totalPoints = group.items.reduce((sum, item) => sum + getRpSpendTotalPoints(item), 0);
...
function getRpSpendRemainingPoints(rpSpend) {
  return Math.max(0, getRpSpendTotalPoints(rpSpend) - toNumber(rpSpend?.redeemedPoints));
}
...
function getRpPointsUsedTotal() {
  return state.rpSpends.reduce((sum, rpSpend) => sum + getRpSpendTotalPoints(rpSpend), 0);
}
