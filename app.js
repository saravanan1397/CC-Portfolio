...
function showPartnerProgramTransferPrompt(initial = {}) {
  return new Promise((resolve) => {
    ...
    const partnerListId = "partnerProgramPromptPartnerList";
    const partnerOptions = getPprExistingPartnerNames()
      .map((partnerName) => `<option value="${escapeAttribute(partnerName)}"></option>`)
      .join("");
      ...
      bodyHtml: `
        <div class="ai-prompt-shell" style="gap:16px;">
          ...
          <label class="field">
            ...
            <input id="${partnerNameId}" type="text" list="${partnerListId}" placeholder="e.g. Marriott, Emirates" value="${escapeAttribute(initial.partnerName || "")}" autocomplete="off" />
            <datalist id="${partnerListId}">${partnerOptions}</datalist>
          </label>
          <label class="field">
...
function getPprSummary() {
  ...
  purchaseGroups.forEach((group) => {
    ...
    if (groupValue > 0) {
      entry.redeemedPoints += groupPoints;
      entry.redeemedValue += groupValue;
      entry.redeemedPurchases += 1;
      entry.cardRedeemedPoints += groupPoints;
      entry.cardRedeemedValue += groupValue;
    } else {
      entry.unredeemedPoints += groupPoints;
      entry.unredeemedValue += groupValue;
      entry.unredeemedPurchases += 1;
      entry.cardUnredeemedPoints += groupPoints;
      entry.cardUnredeemedValue += groupValue;
    }
  ...
  manualEntries.forEach((manualEntry) => {
    const partnerName = manualEntry.partnerName || "Partner";
    const entry = partnerMap.get(partnerName) || {
      partnerName,
      points: 0,
      value: 0,
    ...
    const manualValue = toNumber(manualEntry.value);
    entry.points += manualPoints;
    entry.value += manualValue;
    entry.purchases += 1;
    entry.unredeemedPoints += manualPoints;
    entry.unredeemedValue += manualValue;
    entry.unredeemedPurchases += 1;
    entry.manualUnredeemedPoints += manualPoints;
    entry.manualUnredeemedValue += manualValue;
...
function distributePartnerRedeemedValue(partnerName, totalRedeemedValue, scope = "all") {
  if (!partnerName || totalRedeemedValue < 0) return;

  // Limit edits to the table from which they were initiated. This prevents
  // editing a redeemed partner total from assigning value to unredeemed rows,
  // or editing an unredeemed row from changing an existing redemption.
  const partnerSpends = state.rpSpends.filter(spend => {
    ...
    const hasRedeemedValue = toNumber(spend.pointsValue) > 0;
    if (scope === "redeemed") return hasRedeemedValue;
    if (scope === "unredeemed") return !hasRedeemedValue;
    return true;
  });

  /* legacy predicate retained as a readable reference for the partner filter:
  ...
  */
  ...
  Object.keys(cardContributions).forEach(originatingCardId => {
    // Preserve each redemption row independently. A card can contribute to
    // the same partner more than once; assigning the card total to every row
    // would multiply that card's monetary value.
    cardContributions[originatingCardId].spends.forEach((spend) => {
      const spendPoints = getPartnerProgramPoints(spend);
      spend.pointsValue = totalRedeemedValue * (spendPoints / totalPartnerPoints);
    });
  });
}
...
function showPprValueModal(partnerName, currentValue = 0, scope = "all") {
...
function savePprPartnerValue() {
  ...
  distributePartnerRedeemedValue(pprValueModalPartnerName, value, pprValueModalScope);
...
function openPprManualPointsModal(partnerName = "") {
  ...
  populatePprManualPartnerOptions(partnerName || availablePartners[0] || "");
  if (els.pprManualModalTitle) els.pprManualModalTitle.textContent = "Add Partner Points";
  if (els.pprManualPointsInput) els.pprManualPointsInput.value = "";
  if (els.pprManualValueInput) els.pprManualValueInput.value = "";
  if (els.pprManualNotesInput) els.pprManualNotesInput.value = "";
  if (els.pprManualDateInput) els.pprManualDateInput.value = "";
...
function savePprManualPoints() {
  ...
  if (points < 0 || value < 0) {
    showToast("Points and value cannot be negative.");
    return;
  }

  if (points <= 0 && value <= 0 && !notes && !date) {
    showToast("Enter points, value, date, or notes.");
    return;
  }

  state.pprManualPoints = [
    ...(state.pprManualPoints || []),
    normalizePprManualPoint({
      partnerName,
      points,
      value,
      notes,
  ...
  closePprManualPointsModal();
  render();
  showToast("Partner points added.");
}
    ...
    const tableRows = rows.length
      ? rows.map((row) => {
        const distributableValue = scope === "unredeemed"
          ...
          : scope === "redeemed"
            ? toNumber(row.cardRedeemedValue)
            : toNumber(row.value);
        ...
        <article class="card-row ppr-partner-row" data-partner-name="${escapeAttribute(row.partnerName)}" tabindex="0" role="button" aria-label="View details for ${escapeAttribute(row.partnerName)}" style="padding: 10px 12px; gap: 0; align-items: center; margin: 0;">
          ...
          <div class="money-cell" style="min-width: ${scope === "unredeemed" ? "82px" : "42px"}; padding: 0 6px; justify-content: center;">
            <div class="row-actions inline-actions" style="gap: 4px;">
              <button class="icon-button subtle ppr-edit-value" data-partner-name="${escapeAttribute(row.partnerName)}" data-current-value="${distributableValue}" data-ppr-scope="${scope}" title="Enter redeemed value" aria-label="Enter redeemed value" style="padding: 4px;">
      ...
      <section class="ppr-summary-table-panel" aria-label="${escapeAttribute(title)}">
        ...
        <div class="cards-table ppr-table">
          <div class="table-head" style="padding: 10px 12px; gap: 0; margin: 0; margin-bottom: 0;">
            ...
            <span style="min-width: ${scope === "unredeemed" ? "82px" : "42px"}; padding: 0 6px; text-align: center;"></span>
          </div>
          ${tableRows}
        </div>
      </section>
...
function handlePprWidgetAction(event) {
  ...
  if (editButton) {
    ...
    showPprValueModal(partnerName, currentValue, scope);
    return;
  }
...
function showPprPartnerDetails(partnerName) {
  ...
  const getDetailGroupValue = (group) => group.isManual
    ? toNumber(group.manualEntry?.value)
    : group.items.reduce((groupSum, item) => groupSum + toNumber(item.pointsValue), 0);
