...
function getActivityRpSpendDescription(action, before, after, snapshot) {
  ...
  if (isUnredeemedPointsRecord(spend)) {
    return `${verb} ${formatPoints(points)} of unredeemed points for ${cardName}.`;
  }
...
function collectWidgetActivityChanges(before, after) {
  ...
  compareActivityRecords(before.rpSpends, after.rpSpends, (action, previous, current) => {
    ...
    const description = getActivityRpSpendDescription(action, previous, current, after);
    ...
    if (isPartnerProgramRpSpend(spend)) {
      const partnerName = String(spend.partnerName || spend.purchasedFrom || "partner program").trim() || "partner program";
      push("ppr", action, `${getActivityActionVerb(action)} partner activity for ${partnerName}.`);
    }
  });
  ...
  compareActivityRecords(before.pprManualPoints, after.pprManualPoints, (action, previous, current) => {
    ...
    push("ppr", action, `${getActivityActionVerb(action)} ${formatPoints(points)} of manual points for ${entry.partnerName || "partner program"}.`);
  });
