...
function updatePartnerProgramOriginInfo(selectEl, infoEl, pendingRedeemedPoints = 0) {
  ...
  // Show the real current balance. The existing row's redeemed points are
  // added back only in the edit validation path, never in this display.
  const unredeemedPoints = Math.max(
    0,
    getCardUnredeemedPoints(card) - toNumber(pendingRedeemedPoints)
  );
...
function restoreRpSpendRedemption(rpSpend) {
  ...
  // Rows backed by an explicit Unredeemed Points source are calculated from
  // that source row, so changing the RP spend is enough to restore the balance.
  // Older/manual card balances were deducted directly from card benefits and
  // need the points put back before the replacement or delete is rendered.
...
function getPartnerProgramPoints(rpSpend) {
  if (rpSpend?.pointsReceivedProvided === true) {
    return toNumber(rpSpend.pointsReceived);
  }
